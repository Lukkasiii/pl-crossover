...
export default defineConfig(({ mode }) => ({
  ...
  // e2e/ and e2e-gif/ hold Playwright specs (playwright*.config.ts runs
  // those) -- both frameworks use the *.spec.ts convention, so Vitest's
  // default include glob picks them up too unless told not to, and the two
  // test() globals aren't compatible.
  test: {
    exclude: ['**/node_modules/**', '**/dist/**', 'e2e/**', 'e2e-gif/**'],
  },
}))
