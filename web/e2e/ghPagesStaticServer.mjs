...
//
// GitHub Pages: an exact file match is served as-is; anything else gets the
// *content* of 404.html with a 404 status, and the browser's address bar
// does not change. That "content served, URL untouched" behaviour is what
// makes client-side routing recover on a direct hit -- and it only works if
// dist/404.html exists (see package.json's postbuild:demo).
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    res.writeHead(200, { "content-type": contentType(relative) });
    res.end(data);
  } catch {
    try {
