 ...
 * player controls, prior-weight tuner -- no standings table yet, see
 * StandingsTable's `rows === null` placeholder) and the page after a few
 * frames have streamed in (adds the table body, per-row aria-labels, zone
 * bands). A one-time manual sweep only catches what a person happened to
 * tab through; this catches a regression on every PR instead.
 */
